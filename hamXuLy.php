...
//Các hàm in danh sách sản phẩm
    ...
    function printLisThu($result)
    {
        while ($row = mysqli_fetch_array($result)) {
            ...
            echo '<div class="gia-giamgia">Giá: ' . $row['giaThu'] . ' vnđ</div>';
    ...
    function printListThucAn($result)
    {
        while ($row = mysqli_fetch_array($result)) {
            ...
            echo '<div class="giaSanPham">';
            echo '<div class="gia-giamgia">Giá: ' . $row['giaThucAn'] . ' vnđ</div>';
            echo '</div></a>';
    ...
    function getGioHang($link){
        if (isset($_SESSION['username'])){
            if(!isset($_SESSION['giohang'])){
                $_SESSION['giohang'] = array();
            }
            $IDKH = getIDKhachHang($link,$_SESSION['username']);
            $kh = "select * from giohang where IDKH = '{$IDKH}'";
            ...
            if(mysqli_num_rows($kq)>0){
                while ($row = mysqli_fetch_array($kq)){
                    ...
                    {
                        if($_SESSION['giohang'][$i]['idSP']==$thunuoi['idThu'])
                        {
                            $kt=1;
                            break;
                        }
                    ...
                    if($kt==0){
                        ...
                        $_SESSION['giohang'][$index]['idSP']=$thunuoi['idThu'];
                        $_SESSION['giohang'][$index]['tenSP']=$thunuoi['tenThu'];
                        $_SESSION['giohang'][$index]['giaThu']=$thunuoi['giaThu'];
                        $_SESSION['giohang'][$index]['hinhThu']=$thunuoi['hinhThu'];
